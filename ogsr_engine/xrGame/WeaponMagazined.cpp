...
CWeaponMagazined::~CWeaponMagazined()
{
	...
	HUD_SOUND::DestroySound(sndMisfire);
...
void CWeaponMagazined::Load	(LPCSTR section)
{
	...
	if (pSettings->line_exist(section, "snd_misfire"))
		HUD_SOUND::LoadSound(section, "snd_misfire", sndMisfire, m_eSoundReload);
...
void CWeaponMagazined::FireStart		()
{
	if(IsValid() && !IsMisfire()) 
	{
		if(!IsWorking() || AllowFireWhileWorking())
		{
			...
			if(GetState()==eWPN_FIREMODE_PREV) return;
			if(GetState()==eWPN_FIREMODE_NEXT) return;
...
void CWeaponMagazined::state_Fire	(float dt)
{
	...
	if(const auto E = smart_cast<CEntity*>	(H_Parent()); E && !E->cast_actor())
