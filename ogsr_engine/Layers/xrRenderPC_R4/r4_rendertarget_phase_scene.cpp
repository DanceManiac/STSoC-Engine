...
void	CRenderTarget::phase_scene_begin	()
{
   ...
   if( !RImplementation.o.dx10_gbuffer_opt )
   {
	   if (RImplementation.o.albedo_wo)
		   u_setrt_wetness(rt_Position, rt_Normal, rt_Accumulator, ps_r2_ls_flags_ext.test(R2FLAGEXT_SSLR) && pSettings->r_string_wb("engine_settings", "render_mode") == "r4" ? rt_Wetness : nullptr, pZB);
	   else
		   u_setrt_wetness(rt_Position, rt_Normal, rt_Color, ps_r2_ls_flags_ext.test(R2FLAGEXT_SSLR) && pSettings->r_string_wb("engine_settings", "render_mode") == "r4" ? rt_Wetness : nullptr, pZB);
   }
   else
   {
	   if (RImplementation.o.albedo_wo)
		   u_setrt_wetness(rt_Position, rt_Accumulator, nullptr, ps_r2_ls_flags_ext.test(R2FLAGEXT_SSLR) && pSettings->r_string_wb("engine_settings", "render_mode") == "r4" ? rt_Wetness : nullptr, pZB);
	   else
		   u_setrt_wetness(rt_Position, rt_Color, nullptr, ps_r2_ls_flags_ext.test(R2FLAGEXT_SSLR) && pSettings->r_string_wb("engine_settings", "render_mode") == "r4" ? rt_Wetness : nullptr, pZB);
   }
